...
import { IOrigens } from "../../interfaces/IOrigens";
import { OrigemDialog } from "../../components/dialogs/origem-dialog";

export const OrigensScreen = () => {
  // Aqui é definido as colunas que terão na tabela juntamente com o tamanho que cada coluna deve ocupar
  const OrigensColumns = React.useMemo<ColumnDef<IOrigens>[]>(
    () => [
      {
        id: "select",
        size: 10,
  ...
  return (
    <div className="flex flex-col">
      <Dialog.Root onOpenChange={handleOpenModal}>
        ...
        >
          ...
          <Dialog.Trigger>
            <Button
              ...
              disabled={!selectedRowData.origem_ID}
            >
              <Pencil />
              Editar
            </Button>
        ...
        <DataTable
          ChildModal={OrigemDialog}
          columns={OrigensColumns}
          action={action}
          onRowSelectionChange={handleRowSelectionChange}
          apiUrl="/GetAllOrigens"
        />
      </Dialog.Root>
    </div>
  );
